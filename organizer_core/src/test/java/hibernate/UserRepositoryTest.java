...
public class UserRepositoryTest {
	...
	public void InsertUserWithEvent() {
		...
		events.add(new EventEntitie(EventTypeE.bear, new Date(), new Date(), user));
	...
	public void FindUserByName() {
		...
		List<UserEntitie> dbpost = userRepository.findUserByName(user.getName());
		assertThat(dbpost).isNotNull().isNotEmpty().hasSize(1).containsOnly(user);
	...
	public void FindUserBySurname() {
		...
		assertThat(dbpost).isNotNull().hasSize(2).contains(user1).contains(user2);
	}
}
